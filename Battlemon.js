...
function damage(sender, receiver) {
  const dmg = Math.floor(Math.random() * 22) + 33
  ...
  if (receiver.HP <= 0) {
    setTimeout(function() {
      $('#actionBanner')
        .prop('innerHTML', receiver.name + ' has been defeated')
        .show()
      setTimeout(function() {
        $('#actionBanner')
          .prop('innerHTML', '')
...
function nextPoke(poke) {
  if (poke.ally === 'player') {
    ...
    if (selectedPokemon.playerPosition === 3) {
      victory(true)
    }
    ...
    if (selectedPokemon.enemyPosition === 3) {
      victory(false)
    }
